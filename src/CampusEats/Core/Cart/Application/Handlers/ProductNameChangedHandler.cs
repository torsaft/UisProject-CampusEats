...
public sealed class FoodItemNameChangedHandler : INotificationHandler<ProductNameChanged>
{
    private readonly CampusEatsContext _db;

    public FoodItemNameChangedHandler(CampusEatsContext db)
        => _db = db ?? throw new ArgumentNullException(nameof(db));

    public async Task Handle(ProductNameChanged notification, CancellationToken cancellationToken)
    {
        ...
        foreach(var cart in carts)
        {
            ...
            {
                item.ProductName = notification.NewName;
            }
        }
